...
alph = "aAbBcCdDeEfFgGhHiIjJkKlLmMnNoOpPqQRrSsTtUuVvWwXxYyZz0123456789"
            ...
            if verify == True:
                for num1 in range(len(inp)):
                    if inp[num1] in data_split[num]:####
                        try:
                            if data_split[num][data_split[num].find(inp[num1])-1] in alph and data_split[num].find(inp[num1])-1 > -1:
                                break
                        except:
                            None
                        try:  
                            if data_split[num][data_split[num].find(inp[num1])+len(inp[num1])] in alph:
                                break
                        except:
                            None
